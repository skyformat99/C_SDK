...
###################################################################################
#/*+\NEW\xiongjunqun\2014.03.25\开发open loader*/#
ifeq ($(strip $(AM_OPENAT_LOADER_SUPPORT)), LOADER)
LD_SRC := ${SOFT_WORKDIR}/platform/cust_loader/posloader/posLoader.ld
else
...
ifeq ($(strip $(AM_OPENAT_LOADER_SUPPORT)), LOADER)
LDPPFLAGS += \
        -DAM_OPENAT_LOADER_ROM_BASE=${FLASH_BASE}+${AM_OPENAT_LOADER_ROM_BASE} \
        -DAM_OPENAT_LOADER_ROM_SIZE=${AM_OPENAT_LOADER_ROM_SIZE} \
        -DAM_OPENAT_LOADER_RAM_BASE=${EXT_RAM_BASE}+${AM_OPENAT_LOADER_RAM_BASE} \
        -DAM_OPENAT_LOADER_RAM_SIZE=${AM_OPENAT_LOADER_RAM_SIZE} \
        -DAM_OPENAT_ROM_CFG_BASE=${AM_OPENAT_ROM_CFG_BASE} \
        -DAM_OPENAT_ROM_CFG_SIZE=${AM_OPENAT_ROM_CFG_SIZE} \
        -DAM_OPENAT_ROM_INFO_BASE=${AM_OPENAT_ROM_INFO_BASE} \
        -DAM_OPENAT_ROM_INFO_SIZE=${AM_OPENAT_ROM_INFO_SIZE}
endif
#/*-\NEW\xiongjunqun\2014.03.25\开发open loader*/#
...
ifeq ($(strip $(AM_LUA_ENC_SUPPORT)), TRUE)
	LDPPFLAGS += -DLUA_KEY_BASE=${FLASH_BASE}+${LUA_KEY_BASE} \
    		     -DLUA_KEY_SIZE=${LUA_KEY_SIZE} \
    		     -DAM_LUA_ENC_SUPPORT
endif
#+:\NewReq\brezen\2017.2.22\lua加密#
...
#+:\NewReq\brezen\2017.2.22\lua加密#	
ifeq ($(strip $(AM_LUA_ENC_SUPPORT)), TRUE)
	@${ECHO} "LUA_KEY_BASE:=		 $(LUA_KEY_BASE)"   		>> $(TARGET_FILE)
	@${ECHO} "LUA_KEY_SIZE:=		 $(LUA_KEY_SIZE)"   		>> $(TARGET_FILE)
endif		
#-:\NewReq\brezen\2017.2.22\lua加密#
...
#/*+\NEW\xiongjunqun\2014.03.25\开发open loader*/#
ifeq ($(strip $(AM_OPENAT_LOADER_SUPPORT)), LOADER)
	@${ECHO} "CUST_LOADER_BASE:=	 $(AM_OPENAT_LOADER_ROM_BASE)"     >> $(TARGET_FILE)
	@${ECHO} "CUST_LOADER_SIZE:=	 $(AM_OPENAT_LOADER_ROM_SIZE)"     >> $(TARGET_FILE)
endif	
#/*-\NEW\xiongjunqun\2014.03.25\开发open loader*/#
	@${ECHO} ""                                                 >> $(TARGET_FILE)
	@${ECHO} "#############################################"    >> $(TARGET_FILE)
...
else ifeq ($(strip $(AM_OPENAT_LOADER_SUPPORT)), CUST)
WITH_PLT_LOD_FILE := ${BAS_FINAL}_LOADER_${PLT_LOD_VERSION}.lod
AM_PLT_WITH_LOADER_FILE_DIR=${SOFT_WORKDIR}/platform/cust_loader/loader_plt
AM_PLT_WITH_LOADER_FILE:=`find $(AM_PLT_WITH_LOADER_FILE_DIR) -name "*.lod"`
...
AM_SUBPROJECT_NAME := $(sort $(foreach dir,$(shell find ${AM_SUBPROJECT_DIR}/* -type d -not -path "*.svn*" -and -print | sed 's/.*\///'),$(strip $(dir))))
...
endif #	AM_OPENAT_LOADER_SUPPORT
#/*-\NEW\xiongjunqun\2014.03.25\开发open loader*/#	
...
ifneq "${AM_PLT_ELF_FILE}" ""
#/*+\NEW\xiongjunqun\2014.03.25\开发open loader*/#
ifeq ($(strip $(AM_OPENAT_LOADER_SUPPORT)), )

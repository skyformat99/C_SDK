...
//#define FTPLIB_PORT_MODE_SUPPORT
//#define FTPLIB_SOCKET_SORUSE_SUPPORT
...
int net_read(int fd, char *buf, size_t len)
{
	...
	if ( c == -1 )
	{
	    if ( socket_errno(fd) != EWOULDBLOCK )
		return -1;
	}
	else
	{
...
#define strerror ftp_strerror

const char* ftp_strerror(int errno)
{
    switch(errno)
    {
        case EBADF:
        ...
        case EFAULT:
            return "Bad address specified";
        case EINVAL:
            return "Invalid operation";            
        case EMFILE:
            return "No free socket descriptors";
        case EWOULDBLOCK:
            return "The operation would block";
        case EALREADY:
            return "A previous attempt at this operation is still onging";
        case EDESTADDRREQ:
            return "Socket is not bound ";
        case EMSGSIZE:
            return "The supplied buffer was too large / small";
        case ENOTSUPP:
        ...
        case ECONNRESET:
            return "Remote host has reset the  connection";
        case ENOBUFS:
        ...
        case ECONNREFUSED:
            return "Remote host has rejected the connection";    
        case EHOSTUNREACH:
...
static int socket_wait(netbuf *ctl)
{
    ...
    do
    {
        ...
        printf("%s wait result=%d time=%ds", __func__, rv, tv.tv_sec);
    	if (rv == -1)
    	{
    	    rv = 0;
    	    strncpy(ctl->ctrl->response, strerror(socket_errno(ctl->handle)),
...
static int writeline(const char *buf, int len, netbuf *nData)
{
	...
	if ((*ubp == '\n') && (lc != '\r'))
	{
		...
		{
		    if (ftplib_debug)
			printf("net_write(1) returned %d, err = %s\n", w, strerror(socket_errno(nData->handle)));
		    return(-1);
		}
		nb = 0;
	    }
	...
	if (nb == FTPLIB_BUFSIZ)
	{
	    ...
	    {
		if (ftplib_debug)
		    printf("net_write(2) returned %d, err = %s\n", w, strerror(socket_errno(nData->handle)));
		return(-1);
	    }
	    nb = 0;
	}
	...
	{
	    if (ftplib_debug)
		printf("net_write(3) returned %d, err = %s\n", w, strerror(socket_errno(nData->handle)));
	    return(-1);
	}
    }
    return len;
...
static int readresp(char c, netbuf *nControl)
{
    ...
    if (ftplib_debug > 1)
	fprintf(stdout,"[resp]%s",nControl->response);
...
GLOBALDEF int FtpOptions(int opt, long val, netbuf *nControl)
{
	...
	v = (int) val;
#ifdef FTPLIB_PORT_MODE_SUPPORT   
...
static int FtpOpenPort(netbuf *nControl, netbuf **nData, int mode, int dir)
{
...
#ifdef FTPLIB_SOCKET_SORUSE_SUPPORT     
    ...
    {
        if (ftplib_debug)
            perror("not support FTPLIB_PORT mode");
        return -1;
    }
#endif
...
static int FtpAcceptConnection(netbuf *nData, netbuf *nControl)
{
    ...
    if (i == -1)
    {
        strncpy(nControl->response, strerror(socket_errno(nData->handle),
...
GLOBALDEF int FtpRead(void *buf, int max, netbuf *nData)
{
    ...
    if (nData->buf)
    {
        printf("%s readline", __func__);
        i = readline(buf, max, nData);
    }
    else
    {
        printf("%s socket_wait", __func__);
        i = socket_wait(nData);
    	if (i != 1)
    	    return 0;
